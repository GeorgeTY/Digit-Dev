...
itd_cvter = itd.ImageToDepth("digiteye/calibrate_ballR2_planeElaster")

cv2.namedWindow("y_height")    #原始图像

threshold=195
grad_threshold=100
max_grad_threshold=255
max_threshold=255
...
cv2.createTrackbar("grad_threshold","y_height", grad_threshold,max_grad_threshold,nothing)


while True:
    ret, frame_ = video1.read()
    depth, hm ,ImGradX, ImGradY,ImgGrad= itd_cvter.convert(frame_)
   
    Grad=mat(ImgGrad)
    Min_Grad=Grad.min()
    Max_Grad=Grad.max()
    Grad_=cv2.convertScaleAbs(Grad,alpha=(255/(Max_Grad-Min_Grad)))
    cv2.imshow("Grad",Grad_)
    ret,binary_grad = cv2.threshold(Grad_,grad_threshold,grad_threshold,cv2.THRESH_BINARY)
    cv2.imshow("binary_Grad",binary_grad)

    GradX=mat(ImGradX)
    Min_GradX=GradX.min()
    Max_GradX=GradX.max()
    GradX_=cv2.convertScaleAbs(GradX,alpha=(255/(Max_GradX-Min_GradX)))
    cv2.imshow("GradX",GradX_)
    ret,binary_gradX = cv2.threshold(GradX_,grad_threshold,grad_threshold,cv2.THRESH_BINARY)
    cv2.imshow("binary_GradX",binary_gradX)

    GradY=mat(ImGradY)
    Min_GradY=GradY.min()
    Max_GradY=GradY.max()
    GradY_=cv2.convertScaleAbs(GradY,alpha=(255/(Max_GradY-Min_GradY)))
    cv2.imshow("GradY",GradY_)
    ret,binary_gradY = cv2.threshold(GradY_,grad_threshold,grad_threshold,cv2.THRESH_BINARY)
    cv2.imshow("binary_GradY",binary_gradY)
    ...
    if hm.mean()>0.1:   #当按下传感器时才进行处理
        ...
        grad_threshold=cv2.getTrackbarPos('grad_threshold',"y_height")
